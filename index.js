const listItems = Array.from(document.querySelectorAll(".HorizontalList__Item"));
...
const loginOpen = ()=> document.querySelector("#login").dataset.hidden = "false"
const mouseEnter = (e) => e.target.classList.add("is-expanded");
const mouseLeave = (e) => e.target.classList.remove("is-expanded");
const events = [{ type: "mouseenter", event: mouseEnter },{ type: "mouseleave", event: mouseLeave },];
megaMenu.forEach(el=> {
...
cartButtons.forEach(el=> el.addEventListener("click",()=> toggleHidden("sidebar-cart","hidden")))
console.log(cartOpen,cartClose)
dropDown.forEach(el=> {
  el.addEventListener("mouseenter",(e)=>toggleHidden("DropdownMenu","hidden"))
  el.addEventListener("mouseleave",(e)=>toggleHidden("DropdownMenu","hidden"))
})
